...
final class PairingServer implements Closeable {
 ...
 private volatile long deadline = System.nanoTime() + 10L * 60 * 1000000000;
 private volatile boolean closed;
 private final Set<Socket> clients = Collections.synchronizedSet(new HashSet<Socket>());
 private final ExecutorService requests = new ThreadPoolExecutor(8,8,0,TimeUnit.SECONDS,new ArrayBlockingQueue<Runnable>(8),r->{Thread t=new Thread(r,"tidytivi-pairing-request");t.setDaemon(true);return t;});
 private volatile boolean used;
 private static final String SUCCESS="<!doctype html><meta name='viewport' content='width=device-width,initial-scale=1'><title>Connected</title><body style='background:#0f1620;color:white;font:20px system-ui;text-align:center;padding:60px 20px'>Connected.<p style='font-size:16px;color:#ccc'>You can close this page.</p></body>";
 ...
 void start(){Thread t=new Thread(()->{while(!closed && System.nanoTime()<deadline){
   try {Socket client=server.accept();client.setSoTimeout(15000);clients.add(client);
    try{requests.execute(()->{try(Socket socket=client){handle(socket);}catch(IOException ignored){}finally{clients.remove(client);}});}
    catch(RejectedExecutionException busy){clients.remove(client);client.close();}
 ...
 private void handle(Socket socket)throws IOException{
  ...
  if("POST".equals(request[0])){
   ...
   bytes=new byte[length];new DataInputStream(in).readFully(bytes);
  }
  if(!request[1].equals("/"+token)||closed||System.nanoTime()>=deadline){reply(socket,404,"Setup link expired. Open Connect on your TV.");return;}
  ...
  if("GET".equals(request[0])){reply(socket,200,used?SUCCESS:page);return;}
  ...
  boolean saved=false;String link=null;try{for(String part:new String(bytes,StandardCharsets.UTF_8).split("&")){String[] pair=part.split("=",2);if(pair.length==2&&pair[0].equals("url")){if(link!=null)throw new IOException();link=URLDecoder.decode(pair[1],"UTF-8");}}
   if(link==null||link.trim().length()>4096)throw new IOException();
   synchronized(this){if(closed||System.nanoTime()>=deadline)throw new IOException();if(!used){receiver.save(link.trim());used=true;saved=true;deadline=System.nanoTime()+20L*1000000000;}}
  ...
  try{reply(socket,200,SUCCESS);}
  finally{if(saved)receiver.complete();}
 }
 private void reply(Socket s,int code,String body)throws IOException{
  ...
  OutputStream out=s.getOutputStream();out.write(headers.getBytes(StandardCharsets.US_ASCII));out.write(bytes);out.flush();s.shutdownOutput();
 }
 public void close(){closed=true;try{server.close();}catch(IOException ignored){}synchronized(clients){for(Socket s:clients)try{s.close();}catch(IOException ignored){}clients.clear();}requests.shutdownNow();}
}
